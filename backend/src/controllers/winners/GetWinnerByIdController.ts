...
export class GetWinnerByIdController {
  async execute(req: Request, res: Response) {
    try {
      const validatedParams = idParamSchema.parse(req.params)
      
      const getWinnerByIdService = WinnersFactory.getWinnerByIdService()
      const winner = await getWinnerByIdService.execute({ id: validatedParams.id })
